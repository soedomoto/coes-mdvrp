...
const char * MDVRPSolver::solve(int iterations,
                                int itToMigrate,
                                int redundancy,
                                int itToInnerMig,
                                bool maxMigrations,
    ...
    */
    ...
    const char *out = this->output(population, segment, redundancy, iterations, itToMigrate, itToInnerMig, nMig, nUndoMig, maxMigrations, seed);
...
const char *
MDVRPSolver::output(Population** population, int segment, int redundancy, int iterations, int itToMigrate, int itToInnerMig, int nMig, int nUndoMig, bool maxMigrations, int seed) {
    ...
    for(int i = 1; i < segment*redundancy; ++i) {
        Individual* best = population[i]->best();

        if(best != NULL) {
            outStream << std::to_string(best->depot());
            for (int cust: best->customers()) {
                outStream << " " + std::to_string(cust);
            }

...
data_file = 'm15_n182'
base_data_file = 'm15_n182_base'
simulation_time_scale = float(1)/60
...
sigma_interview_time = 4.13 * 60
# ======================================================================================================================


def setup_redis_cluster_docker():
    cont_names = ['{}_redis_cluster_{}'.format(app_name, i) for i in range(1, 2)]
    ...
    #                cont_names]
    docker_runs = ['docker run -d -P --name {} redis'.format(n) for n in
                   cont_names]
...
class Producer(Thread):
    def __init__(self, brokers, test_num=1):
    ...
    def run(self):
        retval = subprocess.check_call(
            ['../../bin/mdvrp-redis-producer',
             ...
             '-O', os.path.abspath(os.path.join(os.getcwd(), str(self.test_num), 'producer')),
             '-L', os.path.abspath(os.path.join(os.getcwd(), str(self.test_num), '.lock')),
             '-X', str(30)
             ])
    ...
    def run(self):
        retval = subprocess.check_call(
            ['../../bin/mdvrp-redis-consumer',
             ...
             '-D', os.path.abspath(os.path.join(os.getcwd(), data_file)),
             '-C', os.path.abspath(os.path.join(os.getcwd(), './distance_duration_table')),
             '-O', os.path.abspath(os.path.join(os.getcwd(), str(self.test_num), 'consumer')),
             '-L', os.path.abspath(os.path.join(os.getcwd(), str(self.test_num), '.lock')),
...
def main():
    for i in range(1,101):
        ...
        p = Producer(brokers, i)
        c = Consumer(brokers, i)

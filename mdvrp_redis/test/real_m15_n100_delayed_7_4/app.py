...
# Kondisi relatif heterogen, art banyak, dengan geografi susah, jarak antar rumah tangga jauh
# Geographical delay
...
lambda_interview_time = 71.58 * 60
sigma_interview_time = 25.65 * 60
lambda_internode_time = 30.45 * 60
sigma_internode_time = 22.12 * 60
# ======================================================================================================================
    ...
    if not os.path.exists(data_file):
        ...
        for i in range(len(cf.all_bses)):
            cf.all_bses[i].service_time = random_service_time(lambda_interview_time=lambda_interview_time,
                                                              ...
                                                              lambda_internode_time=lambda_internode_time,
                                                              sigma_internode_time=sigma_internode_time)
    ...
    def run(self):
        retval = subprocess.check_call(
            ['../../bin/mdvrp-redis-consumer',
             ...
             '-d', os.path.abspath(os.path.join(os.getcwd(), './delay_list')),
             '-m', str(mu_delay),
             '-s', str(sigma_delay)
             ])

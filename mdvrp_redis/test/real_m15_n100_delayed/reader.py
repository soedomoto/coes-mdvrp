...
cache = RedisCluster(host='172.17.0.9', port=6379)
...
def pubsub_coes():
    for root, dirs, files in os.walk('consumer'):
        ...
        for file in files:
            ...
            if file.endswith('.res'):
                ...
                for li in range(len(routes))[1:]:
                    ...
                    if routes[li] in vst_locs:
                        duplicate_custs.append((vst_by[vst_locs.index(routes[li])], routes[0], routes[li]))

                    vst_locs.append(routes[li])
                    vst_by.append(routes[0])
        ...
        print '============'
        print 'Location count:', len(vst_locs)
        if len(duplicate_custs) > 0:
            print 'Duplicate location(s):', ', '.join(['-'.join(l) for l in duplicate_custs])
        print 'Total costs:', "%.2f" % sum(route_costs.values())
        print 'Rata-rata:', "%.2f" % (sum(route_costs.values()) / len(route_costs.values()))
        print 'Std dev:', "%.2f" % stdev(route_costs.values())
        print ''
